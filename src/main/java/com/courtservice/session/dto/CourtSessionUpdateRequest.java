...
/**
 * Request body for replacing a court session. The hosting court cannot be changed, and
 * {@code bookedCount} and {@code version} are deliberately absent because they are owned by
 * the booking flow and by optimistic locking.
 *
 * @param sessionDate the date of the session
 * @param startTime   the start time
 * @param endTime     the end time, which must be after {@code startTime}
 * @param capacity    the maximum number of bookings, not less than the existing bookings
 * @param openAt      when booking opens
 * @param closeAt     when booking closes, which must be after {@code openAt}
 */
@Schema(description = "Request body for replacing a court session. The hosting court cannot be changed.")
public record CourtSessionUpdateRequest(
        ...
        @Schema(description = "The maximum number of bookings; must be greater than 0 and not less than "
                + "the current bookedCount.", example = "100", minimum = "1",
                requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull
        @Positive
        Integer capacity,
